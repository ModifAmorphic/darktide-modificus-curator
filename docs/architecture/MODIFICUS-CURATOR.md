...
- **The mod root (`--mod-path`) is what Curator owns and writes:** DMF (when
  installed) + user mods + `mods.lst` (the load-order file). Under game-dir
  hosting (the default) the served tree is the same staged projection,
  reached through one Curator-owned link at `<game>/mods` so mods that resolve
  game-directory-relative paths work; the ownership marker inside the staged
  `mods/` proves the link is Curator's, and a foreign entry at that slot is
  never claimed or deleted (the user consents to a rename-aside takeover).
...
- The profile's staged root is the tree Relay's `--mod-path` resolves to (the
...
the profile's staged mod root by, for each enabled mod,
...
regenerated each launch; each pass also rewrites the staging ownership marker
(`staged/mods/.curator.json`: schema, profile id + name, projection timestamp),
which the game-dir hosting ladder reads to prove a link is Curator's. Staging is
a simple loop: base-name collisions are
...
### Game-dir mod hosting

Some mods resolve game-directory-relative paths at runtime and require the
mods tree under the real game directory (`<game>\mods\...` alongside
`<game>\binaries\darktide.exe`). The default launch therefore serves the
staged tree from the game dir through one Curator-owned link, after staging
succeeds and the game binary is known:

1. `GAME_DIR` is derived from the discovered binary
   (`dirname(dirname(binary))`; the Darktide layout is
   `<game>\binaries\darktide.exe`) and validated to exist.
2. `IGameDirModsHost.EnsureHosting(GAME_DIR, stagedRoot)` ensures
   `GAME_DIR\mods` is a link Curator owns, pointing at the active profile's
   staged `mods\` (an NTFS junction on Windows, a symlink on Linux; both
   privilege-free, via the same `StagingLinkCreator` seam staging uses).
   Ownership is decided by the staging marker inside the link's target or a
   target under the profiles root -- never by reparse-ness alone (a user's
   own junction/symlink is foreign). An absent slot creates the link
   silently; a Curator-owned link silently re-points (the link is deleted +
   recreated; the staged tree is never touched through it).
