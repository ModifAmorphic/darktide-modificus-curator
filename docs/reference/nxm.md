...
OS `nxm://` handler is an explicit user action from the Nexus Integrations destination (a
...
explicit user action from the Nexus Integrations destination. Startup only maintains an
