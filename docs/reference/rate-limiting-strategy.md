...
is enforced on save (the Nexus Integrations destination) and at tick time (the runner, via
...
Owned by `NexusConfig` (the bounds) and applied by the Nexus Integrations destination and
`UpdateCheckRunner`.
